...
const Day2CareerMatches = () => {
  ...
  const parseCareerMatches = (text) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('WHY it fits') || line.includes('WHY it fits:')) {
        ...
        if (whyText && currentCareer) currentCareer.why = whyText;
        continue;
      }
      ...
      if (line.includes('Salary range')) {
        ...
        if (salaryText && currentCareer) currentCareer.salary = salaryText;
        continue;
      }
      ...
      if (line.includes('Job demand')) {
        ...
        if (demandText && currentCareer) currentCareer.demand = demandText;
        continue;
      }
  ...
  const getBadgeClass = (percentage) => {
    if (percentage >= 90) return 'badge-top';
    if (percentage >= 80) return 'badge-silver';
    if (percentage >= 70) return 'badge-bronze';
        ...
        if (response.data.matches) {
          ...
          if (parsed.length > 0) {
            ...
            setParsedMatches([{
              ...
              matchPercentage: 85,
              why: response.data.matches.substring(0, 300),
    ...
    <div className="day2-container fade-in">
      ...
      <div className="matches-grid">
        {displayCareers.map((career, index) => {
          const matchPct = career.matchPercentage || 85;
          const badgeLabel = getBadgeLabel(matchPct);
          const badgeClass = getBadgeClass(matchPct);
            ...
            <div key={index} className="match-card">
              ...
              <div className="match-percentage" style={{ color: matchColor }}>
                ...
                <div className="match-bar">
                  <div className="match-bar-fill" style={{ width: `${matchPct}%`, background: matchColor }}></div>
                </div>
              </div>
              ...
              <div className="match-skills">
                {(career.skills || ['Python', 'JavaScript']).slice(0, 6).map((skill, idx) => (
                  <span key={idx} className="skill-tag">{skill}</span>
                ))}
              </div>
              <button 
                className="btn-select" 
                onClick={() => handleSelectCareer(career.title, matchPct)}
              >
                🗺️ Build My Roadmap →
              </button>
            </div>
      ...
      <style>{`
        ...
        .badge-top { background: #4A7C7B; }
        .badge-silver { background: #8B6B4D; }
        .badge-bronze { background: #C4A484; }
        .badge-standard { background: #A89A92; }
        .match-percentage {
          font-size: 1.1rem;
          font-weight: 600;
          margin-bottom: 0.5rem;
